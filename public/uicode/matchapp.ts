...
export class MatchApp extends GameBaseApp {
  ...
  constructor() {
    ...
    this.members_header_toggle_button.addEventListener("click", (e: any) => this.toggleMembersHeader(e));
  ...
  */
  paintGameData(gameDoc: any = null) {
    ...
    if (this.matchBoardRendered !== this.gameData.cardCount) this._renderMatchBoard();
    ...
    if (disabled) this.player_dock_prompt.setAttribute("disabled", true);
    else this.player_dock_prompt.removeAttribute("disabled");
  ...
  _updateCardStatus() {
    for (let c = 0, l = this.gameData.cardCount; c < l; c++) {
      ...
      if (this.gameData.cardIndexesShown[c]) this.matchCards[c].classList.add("previously_shown");
      if (this.gameData.cardIndexesRemoved[c] && c !== this.gameData.previousCard0 &&
        c !== this.gameData.previousCard1) this.matchCards[c].classList.add("matched_hidden");
    }
  ...
  _renderMatchBoard() {
    ...
    if (this.gameData.runningNumberOfSeats > 2) this.cardsPerColumn = 3;
    ...
    for (let cardIndex = 0; cardIndex < this.cardsPerColumn * 8; cardIndex++) {
      ...
      if (cardIndex < this.cardsPerColumn * 2) upperLeft.appendChild(div);
      else if (cardIndex < this.cardsPerColumn * 4) upperRight.appendChild(div);
      else if (cardIndex < this.cardsPerColumn * 6) lowerLeft.appendChild(div);
      else lowerRight.appendChild(div);
  ...
  cardSelected(e: any, div: any, cardIndex: number) {
    ...
    if (div.classList.contains("show_face")) return;
  ...
  _updateCardZoomState() {
    if (!this.upperLeftDisplayCard) return;
  ...
  async _sendSelection() {
    ...
    if (!json.success) {
      ...
      if (this.alertErrors) alert("Failed to send selection: " + json.errorMessage);
      return;
    }
  }
  ...
  async _endTurn() {
    ...
    if (!json.success) {
      ...
      if (this.alertErrors) alert("Failed to resolve selection: " + json.errorMessage);
      return;
    }
  }
